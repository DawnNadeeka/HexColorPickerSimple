#Must install: Tkinter, Pmw, pip

#use this to create the "simple" .exe: pyinstaller --onefile --icon icon.ico --name "HexColorPicker" --add-data "KBCloudyDay.ttf;." HexColorPickerSimple.py
...
try:
    ...
    input("Please install Pmw. To do so, open your command prompt and type 'pip install pmw'.")
    sys.exit()
'''import os
from subprocess import Popen
dir_path = os.path.dirname(os.path.realpath(__file__))+ "\\"

p = Popen("installer.bat", shell=True, cwd=dir_path)
...
def swapModes(): #alters between the two modes: RGB, and HSV
    ...
    if mode == "RGB":
        ...
        labelMid["text"] = "S"
        labelBottom["text"] = "V"
        infoTop.bind(labelTop, "Hue")
        infoMid.bind(labelMid, "Saturation")
        infoBottom.bind(labelBottom, "Value/Brightness")
    else:
        mode = "RGB"
        modeButton["text"] = "HSV"
        ...
        labelMid["text"] = "G"
        labelBottom["text"] = "B"
        infoTop.bind(labelTop, "Red")
        infoMid.bind(labelMid, "Green")
        infoBottom.bind(labelBottom, "Blue")
...
#Help balloons to define R, G, B, etc. 
infoTop = Pmw.Balloon(master)
infoMid = Pmw.Balloon(master)
infoBottom = Pmw.Balloon(master)
infoHex = Pmw.Balloon(master)
infoTop.bind(labelTop, "Red")
infoMid.bind(labelMid, "Green")
infoBottom.bind(labelBottom, "Blue")
infoHex.bind(hexText, "Hexadecimal code\n 6 digits, include '#' symbol")
...
dir_path = os.path.dirname(os.path.realpath(__file__))+ "\\"
p1 = PhotoImage(file = dir_path+'icon.png')
